...
namespace LoyaltyProgramUnitTests
{
    public class UsersEndpoints_should : IDisposable
    {
        ...
        {
            var expected = new LoyaltyProgramUser(0, 
                "Christian", 
                0,
                new LoyaltyProgramSettings());

            var registrationResponse = await sut.PostAsync("/users",
                new StringContent(
            ...
            var actual = await sut.GetAsync($"/users/{newUser?.Id}");
        ...
        {
            var expected = "jane";
            var user = new LoyaltyProgramUser(0,
                "Christian",
                0,
                new LoyaltyProgramSettings());
            ...
            var actual = await sut.PutAsync($"/users/{newUser.Id}",
                new StringContent(
